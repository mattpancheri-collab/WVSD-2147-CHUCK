...
public final class IntakeFactory {
   ...
   * @param stowDeg pivot angle on release (use 0.0 if you want it to go back up)
  ...
  public static Command deployAndIntakeChainPercent(
    ...
    // It never ends on its own; it ends when the overall command ends (button released).
    Command launchFeederAutoStop =
        Commands.run(
                () -> {
    ...
    // Run everything in parallel; this whole command runs until the button is released.
